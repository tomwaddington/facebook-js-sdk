	...
	window.FB || function(window) {
		...
		__d("sdk.DOM", ["Assert", "createArrayFrom", "DOMEventListener", "sdk.domReady", "UserAgent"], function(a, b, c, d, e, f) {
			...
			function p(w, x, y) {
				...
				if(x.querySelectorAll) {
					return h(x.querySelectorAll(y + "." + w))
				}
				var z = x.getElementsByTagName(y), aa = [];
				for(var ba = 0, ca = z.length;ba < ca;ba++) {
					if(m(z[ba], w)) {
						aa[aa.length] = z[ba]
					}
				}
			...
			function q(w, x) {
				...
				if(/backgroundPosition?/.test(x) && /top|left/.test(z)) {
					z = "0%"
				}
				return z
			}
				...
				if(!k.ie()) {
					...
					try {
						...
						if(document.styleSheets[0]) {
							document.styleSheets[0].cssText += w
						}
					}
				}
			}
		...
		__d("XFBML", ["Assert", "FB", "Log", "ObservableMixin", "copyProperties", "createArrayFrom", "dotAccess", "runOnce"], function(a, b, c, d, e, f) {
			...
			function u(x) {
				...
				if(!x.childNodes || x.childNodes.length === 0 || x.childNodes.length == 1 && x.childNodes[0].nodeType == 3 || x.getAttribute("fb-xfbml-state")) {
					return p[y[0]]
				}
			}
			function v(x) {
			...
			function w(x, y, z) {
				...
				ES5(l(x.getElementsByTagName("*")), "forEach", true, function(fa) {
					if(!z && fa.getAttribute("fb-xfbml-state")) {
						return
					}
					var ga = t(fa) || u(fa);
					if(!ga) {
					...
					ba++;
					ca++;
					var ha = ga.ctor || m(h, ga.className.substr(3)), ia = new ha(fa, ga.xmlns, ga.localName, v(fa));
				...
				window.setTimeout(function() {
					if(ba > 0) {
						i.warn("%s tags failed to render in %s ms", ba, ea)
					}
				}, ea);
				da()
			}
			...
			k(r, {registerTag:function(x) {
				o[x.xmlns + ":" + x.localName] = x;
				p[x.xmlns + "-" + x.localName] = x
			}, parse:function(x, y) {
				w(x || document.body, y || function() {
				}, true)
			}, parseNew:function() {
		...
		__d("IframePlugin", ["sdk.DOM", "sdk.Event", "FB", "ObservableMixin", "PluginTags", "QueryString", "sdk.Runtime", "Type", "UrlMap", "sdk.XD", "XFBML", "guid", "insertIframe", "resolveURI"], function(a, b, c, d, e, f) {
			...
			var aa = n.extend({constructor:function(ba, ca, da, ea) {
				...
				ja.style.height = "0px";
				this._element = ba;
				this._config = {root:ja, url:fa + l.encode(ga), name:ia, width:ga.width || 1E3, height:ga.height || 1E3, onload:ES5(this.inform, "bind", true, this, "render")}
			}, process:function() {
