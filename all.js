	...
	(function() {
		...
		if(window.FB && !window.FB.copy) {
			(function() {
				function a(b, c, d, e) {
					for(var f in c) {
						if(d || typeof b[f] === "undefined") {
							b[f] = e ? e(c[f]) : c[f]
						}
					}
					return b
				}
				a(window.FB, {_apiKey:null, _authResponse:null, _userStatus:"unknown", _logging:true, _inCanvas:window.name.indexOf("iframe_canvas") > -1 || window.name.indexOf("app_runner") > -1, _https:function() {
					if(location.protocol == "https:" && window == window.top) {
						return true
					}
					if(/iframe_canvas|app_runner/.test(window.name)) {
				...
				}, _domain:{api:"https://api.facebook.com/", api_read:"https://api-read.facebook.com/", cdn:"http://static.ak.fbcdn.net/", https_cdn:"https://s-static.ak.fbcdn.net/", graph:"https://graph.facebook.com/", staticfb:"http://static.ak.facebook.com/", https_staticfb:"https://s-static.ak.facebook.com/", www:"http://www.facebook.com/", https_www:"https://www.facebook.com/", m:"http://m.facebook.com/", https_m:"https://m.facebook.com/"}, _locale:null, _localeIsRtl:false, getDomain:function(b, c) {
				...
				}, copy:a, create:function(b, c) {
					var d = window.FB, e = b ? b.split(".") : [], f = e.length;
					for(var g = 0;g < f;g++) {
						var h = e[g], i = d[h];
						if(!i) {
							i = c && g + 1 == f ? c : {};
		...
		(function(a) {
			...
			var b = {}, c = {}, d = {}, e = 0, f = 1, g = 2, h = "special", i = "refcount", j = "exports", k = "dependencies", l = "module", m = "waiting", n = "factory", o = undefined, p = "define", q = "global", r = "require", s = "requireLazy", t = "requireDynamic", u = "context";
			function v(ga) {
				if(!b[ga]) {
					throw new Error('Requiring module "' + ga + '" that has not been loaded yet. ' + "Did you forget to run arc build?");
				}
				var ha = b[ga], ia, ja, ka;
				if(ha[m] && ha[h] & g) {
					y()
				}
				if(ha[m]) {
				...
				if(!ha[j]) {
					...
					if(Object.prototype.toString.call(ma) === "[object Function]") {
						var na = [], oa = ha[k], pa = oa.length;
						if(ha[h] & g) {
							pa = Math.min(pa, ma.length)
						}
						for(ja = 0;ja < pa;ja++) {
							ia = oa[ja];
			...
			a[t] = v;
			a[s] = x;
			var ea = false, fa = function(ga, ha, ia, ja) {
				w(ga, ha, ia, ja || g);
				if(b[ga][m] && !ea) {
		...
		__d("ua", [], function(a, b, c, d, e, f) {
			var g = {ie:function() {
				...
				g._populated = true;
				var h = navigator.userAgent, i = /(?:MSIE.(\d+\.\d+))|(?:(?:Firefox|GranParadiso|Iceweasel).(\d+\.\d+))|(?:Opera(?:.+Version.|.)(\d+\.\d+))|(?:AppleWebKit.(\d+(?:\.\d+)?))/.exec(h), j = /(Mac OS X)|(Windows)|(Linux)/.exec(h), k = /\b(iPhone|iP[ao]d)/.exec(h);
				g._win64 = !!/Win64/.exec(h);
				if(i) {
					g._ie = i[1] ? parseFloat(i[1]) : NaN;
					if(g._ie && document.documentMode) {
						g._ie = document.documentMode
					}
					...
					g._ie = g._firefox = g._opera = g._chrome = g._safari = NaN
				}
				if(j) {
					if(j[1]) {
					...
					g._osx = g._windows = g._linux = false
				}
				g._iphone = k
			}};
			a.ua = e.exports = g
		...
		__d("Flash", ["ua", "copyProperties", "QueryString", "guid"], function(a, b, c, d, e, f) {
			...
			var p = {embed:function(q, r, s, t) {
				var u = j();
				q = encodeURI(q);
				s = h({allowscriptaccess:"always", flashvars:t, movie:q}, s || {});
				if(typeof s.flashvars == "object") {
					s.flashvars = i.encode(s.flashvars)
				}
				var v = [];
				for(var w in s) {
					if(s.hasOwnProperty(w) && s[w]) {
						v.push('<param name="' + encodeURI(w) + '" value="' + encodeURI(s[w]) + '">')
					}
				}
				var x = document.createElement("div"), y = "<object " + (g.ie() ? 'classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" ' : 'type="application/x-shockwave-flash"') + 'data="' + q + '" ' + 'id="' + u + '">' + v.join("") + "</object>";
				x.innerHTML = y;
				r.appendChild(x.firstChild);
				o(u);
				return r.firstChild
			}, remove:m, isAvailable:function() {
				var q = "Shockwave Flash", r = "application/x-shockwave-flash", s = "ShockwaveFlash.ShockwaveFlash", t;
				if(navigator.plugins && typeof navigator.plugins[q] == "object") {
					var u = navigator.plugins[q].description;
					if(u && navigator.mimeTypes && navigator.mimeTypes[r] && navigator.mimeTypes[r].enabledPlugin) {
						t = u.match(/\d+/g)
					}
				}
				if(!t) {
		...
		__d("SDK_XD", ["XDM", "Log", "QueryString", "Queue", "resolveWindow", "guid", "XDConfig"], function(a, b, c, d, e, f) {
			...
			var da = {_callbacks:{}, _forever:{}, _channel:t, _origin:u, onMessage:aa, recv:aa, init:function(ea) {
				...
				ga.style.top = "-2000px";
				var ha = h.create({root:ga, channel:t, channelPath:"/" + g.XdUrl + "#", flashUrl:g.Flash.path, whenReady:function(ma) {
					v = ma
		...
		className:"FB.XFBML.Like", supportsWidgetPipe:true}, {localName:"like-box", className:"FB.XFBML.LikeBox"}, {localName:"live-stream", className:"FB.XFBML.LiveStream"}, {localName:"login", className:"FB.XFBML.Login"}, {localName:"login-button", className:"FB.XFBML.LoginButton"}, {localName:"facepile", className:"FB.XFBML.Facepile"}, {localName:"friendpile", className:"FB.XFBML.Friendpile"}, {localName:"name", className:"FB.XFBML.Name"}, {localName:"profile-pic", className:"FB.XFBML.ProfilePic"}, 
		{localName:"question", className:"FB.XFBML.Question"}, {localName:"recommendations", className:"FB.XFBML.Recommendations"}, {localName:"recommendations-bar", className:"FB.XFBML.RecommendationsBar"}, {localName:"registration", className:"FB.XFBML.Registration"}, {localName:"send", className:"FB.XFBML.Send"}, {localName:"serverfbml", className:"FB.XFBML.ServerFbml"}, {localName:"share-button", className:"FB.XFBML.ShareButton"}, {localName:"social-context", className:"FB.XFBML.SocialContext"}, 
		...
		void 0;
		__d("XDConfig", [], {"XdUrl":"connect/xd_arbiter.php?version=3", "Flash":{"path":"https://s-static.facebook.com/rsrc.php/v1/ys/r/WON-TVLCpDP.swf"}, "useCdn":true});
