...
FB.provide("XD", {_origin:null, _transport:null, _callbacks:{}, _forever:{}, _xdProxyUrl:"connect/xd_proxy.php", _openerTransport:null, _openerOrigin:null, _nonOpenerOrigin:null, init:function(b) {
	...
	for(var a = 0, c = f.length;a < c;a++) {
		...
		if(g === "opener" || g === "parent" || g === "top") {
			...
			if(d = /^frames\[['"]?([a-zA-Z0-9-_]+)['"]?\]$/.exec(g)) {
				e = e.frames[d[1]]
			}else {
...
FB.provide("Canvas", {_timer:null, _lastSize:{}, _pageInfo:{clientWidth:0, clientHeight:0, scrollLeft:0, scrollTop:0, offsetLeft:0, offsetTop:0}, _pageInfoPollInterval:200, init:function() {
	...
	if(window.name) {
		...
		var b = {channelUrl:a, frame:window.name, updateInterval:FB.Canvas._pageInfoPollInterval};
	...
	var b = null;
	if(a) {
		b = FB.XD.handler(a, "top.frames[" + window.name + "]", false)
	}
...
FB.provide("Dialog", {_dialogs:{}, _lastYOffset:0, _loaderEl:null, _stack:[], _active:null, get:function(a) {
	...
	var a = FB.UA.nativeApp() ? "" : "<table>" + "	<tbody>" + "		<tr>" + '			<td class="header_left">' + '				<label class="touchable_button">' + '					<input type="submit" value="' + FB.Intl._tx("Cancel") + '"' + '						id="fb_dialog_loader_close"/>' + "				</label>" + "			</td>" + '			<td class="header_center">' + "				<div>" + FB.Intl._tx("Loading...") + "</div>" + "			</td>" + '			<td class="header_right">' + "			</td>" + "		</tr>" + "	</tbody>" + 
	"</table>";
	return FB.Dialog.create({classes:"loading", content:'<div class="dialog_header">' + a + "</div>"})
	...
	if(FB.UA.mobile()) {
		window.scrollTo(0, FB.Dialog._lastYOffset)
	}
}, showLoader:function(a, c) {
	if(!FB.Dialog._loaderEl) {
		FB.Dialog._loaderEl = FB.Dialog._findRoot(FB.UA.mobile() ? FB.Dialog._createMobileLoader() : FB.Dialog._createWWWLoader(c))
	}
	if(!a) {
		a = function() {
		}
...
FB.provide("UIServer", {Methods:{}, _loadedNodes:{}, _defaultCb:{}, _resultToken:'"xxRESULTTOKENxx"', _forceHTTPS:false, genericTransform:function(a) {
	...
	if(b.post) {
		...
		FB.UIServer.setLoadedNode(b, window.open(b.url, b.id, c))
	}
	...
	a.root = FB.Dialog.create({onClose:b, closeIcon:true});
	if(!a.hideLoader) {
	...
	if(FB.UA.mobile()) {
		...
		return{width:window.innerWidth, height:Math.max(window.innerHeight, a ? screen.width : screen.height)}
	}
	...
	if(b.post) {
		FB.Content.insertIframe({url:"about:blank", root:b.root, className:b.className, width:b.size.width, height:b.size.height, onInsert:a, onload:function(c) {
			FB.Content.submitToTarget({url:b.url, target:c.name, params:b.params})
		}})
	}else {
		FB.Content.insertIframe({url:b.url, root:b.root, className:b.className, width:b.size.width, height:b.size.height, name:b.frameName, onInsert:a})
	}
...
FB.provide("", {getLoginStatus:function(a, b) {
	...
	if(!FB._oauth) {
		throw new Error("getAuthResponse called when OAuth2 support is " + "disabled.	Please fix.");
	}
	...
	if(FB._oauth) {
		if(b && b.perms) {
			throw new Error("OAuth2 specification states that 'perms' " + "should now be called 'scope'.	Please update.");
		}
...
FB.provide("Auth", {_callbacks:[], _xdStorePath:"xd_localstorage/", staticAuthCheck:function(b) {
	...
	if(d && d.data && d.data.status && d.data.status == "connected") {
		...
		if(FB._oauth) {
			...
			FB.ui({method:"auth.status", display:"hidden"}, b)
		}
	}
}, setSession:function(e, g) {
	if(FB._oauth) {
		throw new Error("FB.Auth.setSession incompatible with OAuth2.");
	}
	...
	if(!FB._oauth) {
		throw new Error("xdNewHandler should not be invoked unless " + "OAuth2 is being used.");
	}
	return FB.UIServer._xdNextHandler(FB.Auth.xdNewResponseWrapper(b, a), c, d, true)
}, xdNewResponseWrapper:function(b, a) {
	if(!FB._oauth) {
		throw new Error("xdNewResponseWrapper should not be invoked unless " + "OAuth2 is being used.");
	}
	return function(d) {
		if(d.access_token) {
...
FB.provide("UIServer.Methods", {"permissions.request":{size:{width:627, height:326}, transform:function(a) {
	...
	var b = a.cb, c = a.id;
	delete a.cb;
	FB.copy(a.params, {client_id:FB._apiKey, redirect_uri:FB.Auth.xdNewHandler(b, c, "opener"), origin:FB.Auth.getOrigin(), response_type:"token,signed_request"});
	return a
}}, "auth.logout":{url:"logout.php", transform:function(a) {
	if(!FB._apiKey) {
		FB.log("FB.logout() called before calling FB.init().")
	}else {
		if(FB._oauth && !FB._authResponse) {
			FB.log("FB.logout() called without an access token.")
		}else {
			if(!FB._oauth && !FB._session) {
	...
	var b = a.cb, c = a.id;
	delete a.cb;
	FB.copy(a.params, {client_id:FB._apiKey, redirect_uri:FB.Auth.xdNewHandler(b, c, "parent"), origin:FB.Auth.getOrigin(), response_type:"token,signed_request,code"});
	return a
}}});
FB.provide("Cookie", {_domain:null, _enabled:false, setEnabled:function(a) {
	...
	if(!FB._oauth) {
		throw new Error("FB.Cookie.setSignedRequestCookie should only be " + "used with OAuth2.");
	}
	if(!b) {
...
FB.provide("", {initSitevars:{}, init:function(a) {
	...
	if(FB.Canvas && FB.Canvas.EarlyFlush) {
		FB.Canvas.EarlyFlush._maybeSample()
	}
}});
FB.provide("Canvas.EarlyFlush", {_sampleRate:0, _appIdsBlacklist:[], _links:[], COLLECT_AUTOMATIC:0, COLLECT_MANUAL:1, _collectionMode:0, addResource:function(a) {
	...
	if(a != FB.Canvas.EarlyFlush.COLLECT_AUTOMATIC && a != FB.Canvas.EarlyFlush.COLLECT_MANUAL) {
		return false
	}
	FB.Canvas.EarlyFlush._collectionMode = a
}, _maybeSample:function() {
	if(!FB._inCanvas || !FB._apiKey || !FB.Canvas.EarlyFlush._sampleRate) {
		return
	}
	...
	var a = Math.random();
	if(a > 1 / FB.Canvas.EarlyFlush._sampleRate) {
		return
	}
	if(FB.Canvas.EarlyFlush._appIdsBlacklist == "*") {
		return
	}
	if(FB.Array.indexOf(FB.Canvas.EarlyFlush._appIdsBlacklist, parseInt(FB._apiKey, 10)) != -1) {
		return
	}
	window.setTimeout(FB.Canvas.EarlyFlush._sample, 3E4)
}, _sample:function() {
	var b = {object:"data", link:"href", script:"src"};
	if(FB.Canvas.EarlyFlush._collectionMode == FB.Canvas.EarlyFlush.COLLECT_AUTOMATIC) {
		FB.Array.forEach(b, function(c, d) {
			FB.Array.forEach(window.document.getElementsByTagName(d), function(e) {
				if(e[c]) {
					FB.Canvas.EarlyFlush._links.push(e[c])
				}
			})
		})
	}
...
FB.provide("XFBML", {_renderTimeout:3E4, parse:function(d, b) {
	d = d || document.body;
	var c = 1, e = function() {
		c--;
		if(c === 0) {
	...
	FB.Array.forEach(FB.XFBML._tagInfos, function(g) {
		...
		if(a[g.localName] !== undefined) {
			...
			h = FB.XFBML._getDomElements(d, g.xmlns, g.localName)
		}
...
FB.provide("XFBML.IframeWidget", {widgetPipeIframeCount:0, masterWidgetPipeIframe:null, allWidgetPipeIframes:{}, batchWidgetPipeRequests:function() {
	...
	var b = FB.guid();
	FB.Content.insertIframe({url:"about:blank", root:document.getElementById("fb-root") || document.body, name:b, className:"fb_hidden", onload:function() {
		FB.Content.submitToTarget({url:FB._domain.www + "widget_pipe.php", target:b, params:c}, true)
	}})
...
FB.subclass("XFBML.ConnectBar", "XFBML.Element", null, {_initialHeight:null, _initTopMargin:0, _picFieldName:"pic_square", _page:null, _displayed:false, _notDisplayed:false, _container:null, _animationSpeed:0, process:function() {
	...
	b.innerHTML = FB.String.format('<div class="fb_buttons">' + '<a href="#" class="fb_bar_close">' + '<img src="{1}" alt="{2}" title="{2}"/>' + "</a>" + "</div>" + '<a href="{7}" class="fb_profile" target="_blank">' + '<img src="{3}" alt="{4}" title="{4}"/>' + "</a>" + "{5}" + " <span>" + '<a href="{8}" class="fb_learn_more" target="_blank">{6}</a> &ndash; ' + '<a href="#" class="fb_no_thanks">{0}</a>' + "</span>", FB.Intl._tx("No Thanks"), FB.getDomain("cdn") + FB.XFBML.ConnectBar.imgs.buttonUrl, 
	FB.Intl._tx("Close"), d[this._picFieldName] || FB.getDomain("cdn") + FB.XFBML.ConnectBar.imgs.missingProfileUrl, FB.String.escapeHTML(d.first_name), FB.Intl._tx("Hi {firstName}. <strong>{siteName}</strong> is using Facebook to personalize your experience.", {firstName:FB.String.escapeHTML(d.first_name), siteName:FB.String.escapeHTML(d.site_name)}), FB.Intl._tx("Learn More"), d.profile_url, FB.getDomain("www") + "sitetour/connect.php");
	var a = this;
...
FB.subclass("XFBML.EdgeWidget", "XFBML.IframeWidget", null, {_visibleAfter:"immediate", _showLoader:false, setupAndValidate:function() {
	...
	send:this.getAttribute("send"), url_map:this.getAttribute("url_map")};
	return true
}, oneTimeSetup:function() {
...
FB.subclass("XFBML.Send", "XFBML.EdgeWidget", null, {setupAndValidate:function() {
	FB.Dom.addCss(this.dom, "fb_edge_widget_with_comment");
	this._attr = {channel:this.getChannelUrl(), api_key:FB._apiKey, font:this.getAttribute("font"), colorscheme:this.getAttribute("colorscheme", "light"), href:this.getAttribute("href", window.location.href), ref:this.getAttribute("ref")};
	return true
}, getUrlBits:function() {
...
FB.provide("XFBML.Send", {Dimensions:{width:56, height:25}});
...
FB.subclass("XFBML.LoginButton", "XFBML.ButtonElement", null, {setupAndValidate:function() {
	...
	this._attr = {autologoutlink:this._getBoolAttribute("auto-logout-link"), length:this._getAttributeFromList("length", "short", ["long", "short"]), onlogin:this.getAttribute("on-login"), perms:this.getAttribute("perms"), registration_url:this.getAttribute("registration-url"), status:"unknown"};
	...
	return FB.bind(function(c) {
		if(b && this._attr.registration_url && this._attr.status == "notConnected" && c.status == "notConnected") {
			window.top.location = this._attr.registration_url
		}
	...
	if(this._uid == FB.Helper.getLoggedInUser() && this._getBoolAttribute("use-you", true)) {
		...
		if(c) {
			...
			if(this._firstnameonly && c.first_name !== undefined) {
				...
				if(this._lastnameonly && c.last_name !== undefined) {
...
FB.subclass("XFBML.ProfilePic", "XFBML.Element", null, {process:function() {
	...
	var c = this.bind(function(j) {
		...
		if(!i) {
			i = FB.getDomain("cdn") + FB.XFBML.ProfilePic._defPicMap[b]
		}
...
FB.subclass("XFBML.Read", "XFBML.IframeWidget", null, {getUrlBits:function() {
	...
	this._attr = {channel:this.getChannelUrl(), api_key:FB._apiKey, font:this.getAttribute("font"), colorscheme:this.getAttribute("colorscheme", "light"), href:this.getAttribute("href", window.location.href), ref:this.getAttribute("ref"), trigger:e(this.getAttribute("trigger", "")), mintime:parseInt(this.getAttribute("mintime"), 10) || 30};
	...
	var a = FB.bind(function() {
		...
		window.setTimeout(FB.bind(this.arbiterInform, this, "platform/plugins/read/action"), this._attr.mintime * 1E3);
		return true
	}, this);
...
FB.subclass("XFBML.Recommendations", "XFBML.IframeWidget", null, {_visibleAfter:"load", _refreshOnAuthChange:true, setupAndValidate:function() {
	this._attr = {border_color:this.getAttribute("border-color"), colorscheme:this.getAttribute("color-scheme"), filter:this.getAttribute("filter"), font:this.getAttribute("font"), action:this.getAttribute("action"), max_age:this.getAttribute("max_age"), header:this._getBoolAttribute("header"), height:this._getPxAttribute("height", 300), site:this.getAttribute("site", location.hostname), width:this._getPxAttribute("width", 300)};
	return true
}, getSize:function() {
	return{width:this._attr.width, height:this._attr.height}
}, getUrlBits:function() {
	...
	if(this._count.value !== undefined) {
		if(this._count.value.length > 0) {
			...
			if(a > 3) {
				...
				b = a >= 1E7 ? Math.round(a / 1E6) + "M" : a >= 1E4 ? Math.round(a / 1E3) + "K" : a
			}
		}
	}else {
...
FB.subclass("XFBML.SocialBar", "XFBML.EdgeWidget", function(a) {
	if(FB.XFBML.SocialBar.oInstance) {
		return FB.XFBML.SocialBar.oInstance
	}
	this.dom = a;
	...
	if(isNaN(b)) {
		b = parseInt(a.parentNode.offsetWidth, 10)
	}
	return b
	...
	if(a.offsetWidth != c.width) {
		FB.Anim.ate(a, {width:c.width + "px"}, d, function(e) {
			if(c.resetWidth) {
				FB.Dom.setStyle(e, "width", "100%")
			}
		});
		var b = this.dom.getElementsByTagName("iframe");
		FB.Array.forEach(b, function(e) {
			if(e.parentNode.id == "fb_social_bar_container") {
				return
			}
			if(!e._isHidden) {
				...
				e._isHidden = true;
				FB.Anim.ate(e, {height:"0px", width:"0px", right:c.width + "px", left:a.offsetWidth - c.width + "px", opacity:0}, d)
			}else {
				FB.Anim.ate(e, {height:e._isClosed ? "0px" : e._origHeight + "px", width:e._origWidth + "px", right:e._origRight + "px", left:e._origLeft + "px", opacity:100}, d);
				e._isHidden = false
			}
		})
	}
	...
	if(f.name in window.frames) {
		...
		for(var c = 0;c < e.length;c++) {
			...
			if(b.name == f.name) {
				...
				b._isClosed = false;
				FB.Anim.ate(b, {height:f.height, opacity:100})
			}
		}
	}else {
		d.parentNode.appendChild(h);
		var a = this;
		FB.Content.insertIframe({root:h, name:f.name, url:f.src, className:"fb_social_bar_iframe", width:parseInt(f.width, 10), height:0, onload:function(j) {
			...
			FB.Anim.ate(j, {height:f.height, opacity:100});
			j._isClosed = false
		}})
	}
	...
	for(var a = 0;a < b.length;a++) {
		if(b[a].name == c.name) {
			b[a]._isClosed = true;
			FB.Anim.ate(b[a], {height:"0px", opacity:0}, 300, d)
		}
	}
	...
	if(d) {
		...
		FB.Dom.setStyle(c, "width", "35px")
	}
	this._currentZ += parseInt(FB.Dom.getStyle(c, "zIndex"), 10);
	if(isNaN(this._currentZ)) {
		this._currentZ = 99999
	}
	...
	if(!window.XMLHttpRequest) {
		...
		FB.Dom.setStyle(b, this._attr.position, "0px")
	}
	FB.Dom.setStyle(this.dom, "display", "inline");
	function a() {
		this.widgets = {}
	}
	FB.copy(a.prototype, {addWidget:function(e, g, f) {
		this.widgets[e] = FB.copy({widget:g}, f);
		return this
	}, send:function(e) {
		var f = FB.guid();
		var g = FB.copy({widget_pipe:FB.JSON.stringify(this.widgets)}, e);
		FB.Content.insertIframe({url:"about:blank", root:document.getElementById("fb-root") || document.body, name:f, className:"fb_hidden", onload:function() {
			FB.Content.submitToTarget({url:FB._domain.www + "widget_pipe.php", target:f, params:g}, true)
		}})
	}, addSocialBarWidgets:function(e, g) {
		for(var f = 0;f < g.length;f++) {
			this.addWidget(e + ":" + g[f], g[f])
		}
		return this
	}});
	(new a).addSocialBarWidgets(c.name, ["social_bar_controls", "social_bar_profile", "social_bar_like", "social_bar_activity", "social_bar_jewels"]).send({href:window.location, site:this.getAttribute("site", location.hostname), channel:this.getChannelUrl(), api_key:FB._apiKey, locale:FB._locale, sdk:"joey", session_key:FB._session && FB._session.session_key})
}, oneTimeSetup:function() {
	...
	var d = {commentNode:a, externalUrl:c.externalURL, width:330, height:200, masterFrameName:c.masterFrameName, relativeHeightOffset:"0px"};
	...
	if(this._commentWidgetNode) {
		...
		FB.Anim.ate(this._commentWidgetNode, {opacity:0}, 300, FB.bind(function() {
			this._commentWidgetNode.style.display = "none"
		}, this))
	}
}, _handleEdgeCommentDialogShow:function() {
	if(this._commentWidgetNode) {
		this._commentWidgetNode.style.display = "block";
		FB.Anim.ate(this._commentWidgetNode, {opacity:100}, 500)
	}
}, _handleEdgeCommentDialogDismissal:function(a) {
	if(this._commentWidgetNode) {
