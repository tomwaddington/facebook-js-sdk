window.FB || function() {
	...
	(function() {
		...
		__d("applyWithGuard", ["UserAgent"], function(a, b, c, d, e, f) {
			var g = b("UserAgent"), h;
			function i(k) {
				var l = Array.prototype.slice.call(arguments.callee.caller.caller.arguments), m = {line:k.lineNumber || k.line, message:k.message, name:k.name, script:k.fileName || k.sourceURL || k.script, stack:k.stackTrace || k.stack, args:ES5("JSON", "stringify", false, l).substring(0, 200)};
				m._originalError = k;
		...
		__d("FB", ["applyWithGuard", "guid", "dotAccess", "copyProperties", "safeEval", "GlobalCallback", "Log", "QueryString", "UrlMap", "SDKConfig"], function(a, b, c, d, e, f) {
			var g = c("SDKConfig"), h = b("applyWithGuard"), i = b("guid"), j = b("dotAccess"), k = b("copyProperties"), l = b("safeEval"), m = b("GlobalCallback"), n = b("Log"), o = b("QueryString"), p = b("UrlMap"), q, r, s, t = j(g, "api.mode"), u = {};
			q = FB;
			r = window.FB = {};
			...
			function v(ca) {
				k(ca, {appId:q._apiKey});
				(new Image).src = o.appendToUrl(p.resolve("www", true) + "/common/scribe_endpoint.php", {c:"jssdk_error", m:ES5("JSON", "stringify", false, ca)})
			}
			var w = g.errorHandling.rate;
			if(w && Math.floor(Math.random() * 100) + 1 < w) {
				h.setErrorHandler(function(ca) {
					typeof console !== "undefined" && typeof bagofholding === "function";
					delete ca._originalError;
					v({error:ca.name || ca.message, extra:ca});
					throw ca;
				})
			}
			function x(ca, da) {
				var ea = ca ? j(q, ca, true) : q, fa = ca ? j(r, ca, true) : r;
				ES5(ES5("Object", "keys", false, da), "forEach", true, function(ga) {
					var ha = (ca ? ca + "." : "") + ga, ia = da[ga], ja, ka;
					ea[ga] = ia;
							...
							ka = function() {
								if(ja === "warn") {
									...
									if(!u.hasOwnProperty(ha)) {
										v({error:"Private method used", extra:{args:ha}});
										u[ha] = true
									}
								}
								var la = ES5(Array.prototype.slice.call(arguments), "map", true, function(ma) {
									return typeof ma === "function" && /^function/.test(ma.toString()) ? function() {
										var na = arguments;
										setTimeout(function() {
											ma.apply(null, na)
										}, 0)
			...
			var y = /iframe_canvas|app_runner/.test(window.name), z = /dialog/.test(window.name), aa = function() {
				if(location.protocol == "https:" && (window == top || !(y || z))) {
					return true
				}
			...
			function ba(ca, da, ea, fa) {
				for(var ga in da) {
					if(ea || typeof ca[ga] === "undefined") {
						ca[ga] = fa ? fa(da[ga]) : da[ga]
					}
				}
				return ca
			}
			k(q, {_apiKey:null, _authResponse:null, _userStatus:"unknown", _logging:true, _inCanvas:y, _https:aa, onlyUseHttps:function() {
			...
			}, provide:x, guid:i, log:function(ca) {
				if(q._logging) {
					if(window.Debug && window.Debug.writeln) {
						window.Debug.writeln(ca)
					}else {
						if(window.console) {
							window.console.log(ca)
						}
					}
				}
				if(q.Event) {
					q.Event.fire("fb.log", ca)
				}
			}, $:function(ca) {
		...
		{localName:"privacy-selector", className:"FB.XFBML.PrivacySelector"}, {localName:"profile-pic", className:"FB.XFBML.ProfilePic"}, {localName:"recommendations", className:"FB.XFBML.Recommendations"}, {localName:"recommendations-bar", className:"FB.XFBML.RecommendationsBar"}, {localName:"registration", className:"FB.XFBML.Registration"}, {localName:"send", className:"FB.XFBML.Send"}, {localName:"share-button", className:"FB.XFBML.ShareButton"}, {localName:"social-context", className:"FB.XFBML.SocialContext"}, 
		{localName:"subscribe", className:"FB.XFBML.Subscribe"}], _widgetPipeEnabledTagCount:0, _widgetPipeIsEnabled:function() {
		...
		FB.subclass("XFBML.LoginButton", "XFBML.IframeWidget", null, {_showLoader:false, setupAndValidate:function() {
			this._attr = {channel:this.getChannelUrl(), colorscheme:this.getAttribute("colorscheme"), max_rows:this.getAttribute("max-rows"), width:this._getPxAttribute("width"), show_faces:this._getBoolAttribute("show-faces"), show_login_face:this._getBoolAttribute("show-login-face"), size:this.getAttribute("size"), login_text:this.dom.textContent || this.dom.innerText, registration_url:this.getAttribute("registration-url"), one_click:this.getAttribute("one-click"), scope:this.getAttribute("scope") || 
