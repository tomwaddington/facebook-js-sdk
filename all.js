window.FB || function() {
	...
	(function() {
		(function(a) {
			...
			var b = {}, c = {}, d = {}, e = 0, f = 1, g = 2, h = Object.prototype.hasOwnProperty;
			function i(t) {
				var u = b[t], v, w, x;
				if(!b[t]) {
			...
			a.requireDynamic = i;
			a.requireLazy = k;
			i.__debug = {modules:b, deps:c};
			var r = false, s = function(t, u, v, w) {
				j(t, u, v, w || g);
				if(b[t].waiting && !r) {
		...
		__d("applyWithGuard", ["UserAgent"], function(a, b, c, d, e, f) {
			var g = b("UserAgent"), h;
			function i(k) {
				var l = {line:k.lineNumber || k.line, message:k.message, name:k.name, script:k.fileName || k.sourceURL || k.script, stack:k.stackTrace || k.stack};
				l._originalError = k;
		...
		__d("FB", ["applyWithGuard", "guid", "dotAccess", "copyProperties", "QueryString", "UrlMap", "SDKConfig"], function(a, b, c, d, e, f) {
			var g = c("SDKConfig"), h = b("applyWithGuard"), i = b("guid"), j = b("dotAccess"), k = b("copyProperties"), l = b("QueryString"), m = b("UrlMap"), n, o;
			if(g.seal) {
				n = FB;
				o = window.FB = {}
			}else {
			...
			if(p && Math.floor(Math.random() * 100) + 1 < p) {
				h.setErrorHandler(function(u) {
					var v = l.appendToUrl(m.resolve("www", true) + "/common/scribe_endpoint.php", {c:"jssdk_error", d:ES5("JSON", "stringify", false, {error:u.name || u.message, extra:u})});
					(new Image).src = v;
					throw u;
				})
			}
			...
			var r = /iframe_canvas|app_runner|dialog/.test(window.name), s = function() {
				if(location.protocol == "https:" && (window == top || !r)) {
					return true
				}
			...
			function t(u, v, w, x) {
				for(var y in v) {
					if(w || typeof u[y] === "undefined") {
						u[y] = x ? x(v[y]) : v[y]
					}
				}
				return u
			}
			k(n, {_apiKey:null, _authResponse:null, _userStatus:"unknown", _logging:true, _inCanvas:r, _https:s, onlyUseHttps:function() {
				return n._https === true
			}, onlyUseHttp:function() {
				return n._https === false && location.protocol == "http:"
			}, _locale:null, _localeIsRtl:false, getDomain:function(u, v) {
			...
			}, copy:t, create:function(u, v) {
				var w = u.split(".");
				u = w.pop();
				var x = w.length ? j(n, w.join("."), true) : n;
				return u in x ? x[u] : x[u] = v || {}
			}, provide:q, guid:i, log:function(u) {
				if(n._logging) {
				...
				if(n.Event) {
					n.Event.fire("fb.log", u)
				}
			}, $:function(u) {
				return document.getElementById(u)
			}, dotAccess:j, applyWithGuard:h});
			if(o) {
				k(o, {provide:q})
			}
			e.exports = n
		...
		FB.provide("ApiServer", {METHODS:["get", "post", "delete", "put"], _readOnlyCalls:{fql_query:true, fql_multiquery:true, friends_get:true, notifications_get:true, stream_get:true, users_getinfo:true}, graph:function() {
			...
			var f = FB.guid(), g = document.getElementsByTagName("script")[0], h = document.createElement("script"), i = window.FB._callbacks || (window.FB._callbacks = {});
			d = FB.copy({}, d);
			if(a === "graph" && c !== "get") {
				d.method = c
			}
			...
			if(!window.FB_OnXdHttpResult) {
				window.FB_OnXdHttpResult = function(g, h) {
					FB.applyWithGuard(function() {
			...
			FB.Flash.onReady(function() {
				...
				var g = FB.getDomain(a) + b, h = FB.QS.encode(d);
				if(c === "get") {
					if(g.length + h.length > 2E3) {
						if(a === "graph") {
							d.method = "get"
						}
						c = "post";
		...
		__d("CORSRequest", ["applyWithGuard", "QueryString"], function(a, b, c, d, e, f) {
			var g = b("applyWithGuard"), h = b("QueryString");
			function i(l, m) {
				if(!window.XMLHttpRequest) {
					return null
				}
			...
			function w(aa, ba, ca, da) {
				...
				var ga = function(ka) {
					...
					if(r && ka && typeof ka == "object") {
						if(ka.error) {
							if(ka.error == "invalid_token" || ka.error.type == "OAuthException" && ka.error.code == 190) {
								la = true
							}
						}else {
							if(ka.error_code) {
								if(ka.error_code == "190") {
									la = true
								}
							}
						}
		...
		FB.provide("EventProvider", {subscribers:function() {
			...
			if(a.addEventListener) {
				...
				if(a.attachEvent) {
					a.attachEvent("on" + event, b.wrapper)
				}
			}
			...
			if(a.removeEventListener) {
				...
				if(a.detachEvent) {
					a.detachEvent("on" + event, b.wrapper)
				}
			}
		}});
		...
		__d("XDM", ["DOMEventListener", "DOMWrapper", "Flash", "Log", "copyProperties", "guid"], function(a, b, c, d, e, f) {
			var g = b("DOMEventListener"), h = b("DOMWrapper"), i = b("Flash"), j = b("Log"), k = b("copyProperties"), l = b("guid"), m = {}, n = {transports:[]}, o = h.getWindow();
			function p(r) {
			...
			var q = {register:function(r, s) {
				...
				if(!r.whenReady) {
					r.whenReady = bagofholding
				}
				if(!r.onMessage) {
					r.onMessage = bagofholding
				}
			...
			q.register("flash", function() {
				var r = false, s, t = {}, u = false, v = 15E3, w;
				return{isAvailable:function() {
					...
					var z = x.root.appendChild(o.document.createElement("div")), aa = l();
					t[aa] = function() {
						clearTimeout(w);
					...
					s = i.embed(x.flashUrl, z, null, {protocol:location.protocol.replace(":", ""), host:location.host, callback:"FB_XDM_CALLBACKS." + aa, log:u});
		...
		__d("SDK_XD", ["applyWithGuard", "guid", "resolveWindow", "FB", "XDM", "Log", "QueryString", "Queue", "XDConfig"], function(a, b, c, d, e, f) {
			var g = c("XDConfig"), h = b("applyWithGuard"), i = b("guid"), j = b("resolveWindow"), k = b("FB"), l = b("XDM"), m = b("Log"), n = b("QueryString"), o = b("Queue"), p = new o, q = new o, r = new o, s, t, u = i(), v = i(), w = location.protocol + "//" + location.host, x, y = false, z = {};
			function aa(ga) {
			...
			function ca(ga, ha) {
				h(function() {
					...
					if(ga.access_token) {
						k._https = /^https/.test(w)
					}
					if(ga.cb) {
						var ja = k.XD._callbacks[ga.cb];
						if(!k.XD._forever[ga.cb]) {
							delete k.XD._callbacks[ga.cb]
						}
						if(ja) {
							ja(ga)
						}
			...
			var da = function() {
				var ga = document.createElement("form"), ha = ga.appendChild(document.createElement("input")), ia;
				ha.name = i();
			...
			var fa = {_callbacks:{}, _forever:{}, _channel:v, _origin:w, onMessage:ca, recv:ca, init:function(ga) {
				if(y) {
					return
				}
				var ha = ga ? /\/\/.*?(\/[^#]*)/.exec(ga)[1] : location.pathname + location.search;
				ha += (~ES5(ha, "indexOf", true, "?") ? "&" : "?") + "fb_xd_fragment#xd_sig=" + u + "&";
				var ia = k.Content.appendHidden(document.createElement("div")), ja = l.create({root:ia, channel:v, channelPath:"/" + g.XdUrl + "#", flashUrl:g.Flash.path, whenReady:function(oa) {
					x = oa
				}, onMessage:ca}), ka = {channel:v, origin:location.protocol + "//" + location.host, channel_path:ha, transport:ja}, la = g.XdUrl + "#" + n.encode(ka), ma = g.useCdn ? k._domain.staticfb : "http://www.facebook.com/", na = g.useCdn ? k._domain.https_staticfb : "https://www.facebook.com/";
				if(!k.onlyUseHttps()) {
					s = ea({url:ma + la, name:"fb_xdm_frame_http", root:ia})
				}
				t = ea({url:na + la, name:"fb_xdm_frame_https", root:ia});
				y = true
				...
				ja = ja || i();
				if(ia) {
					k.XD._forever[ja] = true
				}
				k.XD._callbacks[ja] = ga;
				var ka = location.protocol == "https:" ? k._domain.https_staticfb : k._domain.staticfb, la = g.useCdn ? ka : location.protocol + "//www.facebook.com/";
				return la + g.XdUrl + "#" + n.encode({cb:ja, origin:w + "/" + v, domain:location.hostname, relation:ha || "opener"})
			}};
			(function() {
				var ga = location.href.match(/[?&]fb_xd_fragment#(.*)$/);
				if(ga) {
					document.documentElement.style.display = "none";
					var ha = n.decode(ga[1]), ia = j(ha.xd_rel);
		...
		FB.provide("XFBML", {_renderTimeout:3E4, getElements:function(a, b, c) {
			...
			if(d) {
				...
				var e = function() {
					var f = FB.dotAccess(FB, b.className.replace(/^FB\./, "")), g = false, h = true, i = false, j = false, k = b.className === "FB.XFBML.AddToTimeline";
					...
					if(g) {
						h = !!h;
						i = !!i;
						var l = {show_faces:h, show_login_face:i, add_to_profile:k, mode:mode}, m = FB.XFBML.getAttr(a, "scope") || FB.XFBML.getAttr(a, "perms");
						if(m) {
							l.scope = m
						}
						d.setExtraParams(l)
				...
				if(FB.CLASSES[b.className.substr(3)]) {
					...
					FB.log("Tag " + b.className + " was not found.")
				}
			}
...
FB.provide("Arbiter", {"_canvasProxyUrl":"connect/canvas_proxy.php?version=6"}, true);
