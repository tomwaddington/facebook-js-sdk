window.FB || function() {
	var ES5 = function() {
		...
		__d("JSON3", [], function(a, b, c, d, e, f) {
				...
				if(j) {
					...
					try {
						switch(false) {
							...
							;
							case f.stringify({result:[m, true, false, null, "\x00\b\n\f\r\t"]}) == l:
							;
			...
			if(!k) {
				f.parse = function() {
					var l = {"\\":"\\", '"':'"', "/":"/", b:"\b", t:"\t", n:"\n", f:"\f", r:"\r"}, m = "".constructor.fromCharCode;
					function n(s) {
						this.source = s;
						this.index = 0
					}
	...
	(function() {
		...
		__d("GlobalCallback", ["applyWithGuard", "dotAccess"], function(a, b, c, d, e, f) {
			var g = b("applyWithGuard"), h = b("dotAccess"), i, j, k = 0, l = {setPrefix:function(m) {
				i = h(window, m, true);
				j = m
			}, create:function(m) {
				if(!i) {
		...
		__d("FB", ["applyWithGuard", "guid", "dotAccess", "copyProperties", "safeEval", "GlobalCallback", "sdk.Insights", "Log", "sdk.Scribe", "QueryString", "UrlMap", "SDKConfig"], function(a, b, c, d, e, f) {
			var g = c("SDKConfig"), h = b("applyWithGuard"), i = b("guid"), j = b("dotAccess"), k = b("copyProperties"), l = b("safeEval"), m = b("GlobalCallback"), n = b("sdk.Insights"), o = b("Log"), p = b("sdk.Scribe"), q = b("QueryString"), r = b("UrlMap"), s, t, u, v = j(g, "api.mode"), w = {};
			s = FB;
			t = window.FB = {};
			...
			if(x && Math.floor(Math.random() * 100) + 1 < x) {
				h.setErrorHandler(function(da) {
					typeof console !== "undefined" && typeof bagofholding === "function";
					delete da._originalError;
					p.log("jssdk_error", {appId:s._apiKey, error:da.name || da.message, extra:da});
					throw da;
				})
			}
			function y(da, ea) {
				var fa = da ? j(s, da, true) : s, ga = da ? j(t, da, true) : t;
				ES5(ES5("Object", "keys", false, ea), "forEach", true, function(ha) {
					var ia = (da ? da + "." : "") + ha, ja = ea[ha], ka, la;
					fa[ha] = ja;
						...
						default:
							la = function() {
								...
								var ma = ES5(Array.prototype.slice.call(arguments), "map", true, function(na) {
									return typeof na === "function" && /^function/.test(na.toString()) ? function() {
										var oa = arguments;
										setTimeout(function() {
											na.apply(null, oa)
										}, 0)
			...
			var z = /iframe_canvas|app_runner/.test(window.name), aa = /dialog/.test(window.name), ba = function() {
				if(location.protocol == "https:" && (window == top || !(z || aa))) {
					return true
				}
			...
			function ca(da, ea, fa, ga) {
				for(var ha in ea) {
					if(fa || typeof da[ha] === "undefined") {
						da[ha] = ga ? ga(ea[ha]) : ea[ha]
					}
				}
				return da
			}
			k(s, {_apiKey:null, _authResponse:null, _userStatus:"unknown", _logging:true, _inCanvas:z, _https:ba, onlyUseHttps:function() {
			...
			}, provide:y, guid:i, log:function(da) {
				if(s._logging) {
		...
		__d("legacy:fb.api", ["ApiClient", "FB", "SDKConfig"], function(a, b, c, d) {
			var e = c("SDKConfig"), f = b("ApiClient"), g = b("FB");
			f.setDefaultParams({sdk:"joey"});
			function h() {
				var i = g.getAccessToken(), j = function() {
					if(i == g.getAccessToken()) {
		...
		__d("XDM", ["copyProperties", "guid", "DOMEventListener", "DOMWrapper", "Flash", "Log", "UserAgent"], function(a, b, c, d, e, f) {
			...
			var r = {register:function(s, t) {
				...
				if(!s.whenReady) {
					s.whenReady = bagofholding
				}
				if(!s.onMessage) {
					s.onMessage = bagofholding
				}
			...
			r.register("flash", function() {
				var s = false, t, u = {}, v = false, w = 15E3, x;
				return{isAvailable:function() {
			...
			g(i.prototype, {stub:function(j) {
				...
				if(m) {
					q = function(t, u) {
						...
						v[t] = u;
						setTimeout(function() {
							o._write(ES5("JSON", "stringify", false, v), {ref:k})
						}, 0)
					}
				}else {
					q = function() {
		...
		__d("SDK_XD", ["applyWithGuard", "guid", "resolveWindow", "FB", "XDM", "Log", "QueryString", "Queue", "URL", "JSONRPC", "XDConfig"], function(a, b, c, d, e, f) {
			var g = c("XDConfig"), h = b("applyWithGuard"), i = b("guid"), j = b("resolveWindow"), k = b("FB"), l = b("XDM"), m = b("Log"), n = b("QueryString"), o = b("Queue"), p = b("URL"), q = b("JSONRPC"), r = new o, s = new o, t = new o, u, v, w = i(), x = i(), y = location.protocol + "//" + location.host, z, aa = false, ba = {}, ca;
			function da(ka) {
			...
			function fa(ka, la) {
				h(function() {
					if(la && la !== "native" && !p(la).isFacebookURL()) {
						return
					}
					if(typeof ka == "string") {
						if(/^FB_RPC:/.test(ka)) {
					...
					if(ka.access_token) {
						k._https = /^https/.test(y)
					}
					if(ka.cb) {
						var na = k.XD._callbacks[ka.cb];
						if(!k.XD._forever[ka.cb]) {
							delete k.XD._callbacks[ka.cb]
						}
						if(na) {
							na(ka)
						}
			...
			var ga = function() {
				var ka = document.createElement("form"), la = ka.appendChild(document.createElement("input")), ma;
				la.name = i();
			...
			var ja = {rpc:ca, _callbacks:{}, _forever:{}, _channel:x, _origin:y, onMessage:fa, recv:fa, init:function(ka, la) {
				if(aa) {
					return
				}
				var ma = ka ? /\/\/.*?(\/[^#]*)/.exec(ka)[1] : location.pathname + location.search;
				ma += (~ES5(ma, "indexOf", true, "?") ? "&" : "?") + "fb_xd_fragment#xd_sig=" + w + "&";
				var na = k.Content.appendHidden(document.createElement("div")), oa = l.create({root:na, channel:x, channelPath:"/" + g.XdUrl + "#", flashUrl:g.Flash.path, whenReady:function(pa) {
					z = pa;
					var qa = {channel:x, origin:location.protocol + "//" + location.host, channel_path:ma, transport:oa, xd_name:la}, ra = g.XdUrl + "#" + n.encode(qa), sa = g.useCdn ? k._domain.staticfb : "http://www.facebook.com/", ta = g.useCdn ? k._domain.https_staticfb : "https://www.facebook.com/";
					if(!k.onlyUseHttps()) {
						u = ha({url:sa + ra, name:"fb_xdm_frame_http", root:na})
					}
					v = ha({url:ta + ra, name:"fb_xdm_frame_https", root:na})
				}, onMessage:fa});
				aa = true
			}, sendToFacebook:ia, handler:function(ka, la, ma, na) {
				na = na || i();
				if(ma) {
					k.XD._forever[na] = true
				}
				k.XD._callbacks[na] = ka;
				var oa = location.protocol == "https:" ? k._domain.https_staticfb : k._domain.staticfb, pa = g.useCdn ? oa : location.protocol + "//www.facebook.com/";
				return pa + g.XdUrl + "#" + n.encode({cb:na, origin:y + "/" + x, domain:location.hostname, relation:la || "opener"})
			}};
			(function() {
				var ka = location.href.match(/[?&]fb_xd_fragment#(.*)$/);
				if(ka) {
					document.documentElement.style.display = "none";
					var la = n.decode(ka[1]), ma = j(la.xd_rel);
		...
		FB.provide("Canvas", {_timer:null, _lastSize:{}, _isTabIframe:!!~ES5(window.name, "indexOf", true, "app_runner_"), _pageInfo:{clientWidth:0, clientHeight:0, scrollLeft:0, scrollTop:0, offsetLeft:0, offsetTop:0}, getPageInfo:function(a) {
			...
			if(FB.initSitevars.rpc) {
				FB.XD.rpc.remote.scrollTo({x:a, y:b})
			}else {
			...
			var a = document.body, b = document.documentElement, c = 0, d = Math.max(a.offsetTop, 0), e = Math.max(b.offsetTop, 0), f = a.scrollHeight + d, g = a.offsetHeight + d, h = b.scrollHeight + e, i = b.offsetHeight + e, j = Math.max(f, g, h, i);
			if(a.offsetWidth < a.scrollWidth) {
				c = a.scrollWidth + a.offsetLeft
			}else {
				ES5(FB.Array.toArray(a.childNodes), "forEach", true, function(k) {
		...
		FB.provide("UIServer", {Methods:{}, _loadedNodes:{}, _defaultCb:{}, _resultToken:'"xxRESULTTOKENxx"', _forceHTTPS:false, genericTransform:function(a) {
			...
			var b = a.params.method, c = FB.initSitevars.useAsync ? FB._inCanvas : FB.Canvas.isTabIframe();
			if(!c) {
		...
		FB.Class("XFBML.IframePlugin", function(a, b) {
			...
			var l = {url:h + FB.QS.encode(i), name:k, width:0, height:0, onload:ES5(this.fire, "bind", true, this, "render")};
		...
		}, _tagInfos:[{localName:"activity", className:"FB.XFBML.Activity"}, {localName:"activity-log", className:"FB.XFBML.ActivityLog"}, {localName:"add-profile-tab", className:"FB.XFBML.AddProfileTab"}, {localName:"add-to-timeline", className:"FB.XFBML.AddToTimeline"}, {localName:"bookmark", className:"FB.XFBML.Bookmark"}, {localName:"comments", className:"FB.XFBML.Comments"}, {localName:"comments-count", className:"FB.XFBML.CommentsCount"}, {localName:"connect-bar", className:"FB.XFBML.ConnectBar"}, 
		{localName:"create-event-button", className:"FB.XFBML.CreateEventButton"}, {localName:"fan", className:"FB.XFBML.Fan"}, {localName:"like", className:"FB.XFBML.Like", supportsWidgetPipe:true}, {localName:"like-box", className:"FB.XFBML.LikeBox"}, {localName:"live-stream", className:"FB.XFBML.LiveStream"}, {localName:"login", className:"FB.XFBML.Login"}, {localName:"login-button", className:"FB.XFBML.LoginButton"}, {localName:"facepile", className:"FB.XFBML.Facepile"}, {localName:"friendpile", 
		className:"FB.XFBML.Friendpile"}, {localName:"name", className:"FB.XFBML.Name"}, {localName:"page-events", className:"FB.XFBML.PageEvents"}, {localName:"privacy-selector", className:"FB.XFBML.PrivacySelector"}, {localName:"profile-pic", className:"FB.XFBML.ProfilePic"}, {localName:"recommendations", className:"FB.XFBML.Recommendations"}, {localName:"recommendations-bar", className:"FB.XFBML.RecommendationsBar"}, {localName:"registration", className:"FB.XFBML.Registration"}, {localName:"send", 
		className:"FB.XFBML.Send"}, {localName:"share-button", className:"FB.XFBML.ShareButton"}, {localName:"social-context", className:"FB.XFBML.SocialContext"}, {localName:"subscribe", className:"FB.XFBML.Subscribe"}], _widgetPipeEnabledTagCount:0, _widgetPipeIsEnabled:function() {
		...
		FB.provide("Data", {query:function(a, b) {
			...
			FB.api("/fql", "GET", f, function(g) {
				if(g.error) {
					...
					ES5(g.data, "forEach", true, function(h) {
		...
		FB.subclass("XFBML.ActivityLog", "XFBML.IframeWidget", null, {setupAndValidate:function() {
			this._attr = {channel:this.getChannelUrl(), api_key:FB._apiKey, font:this.getAttribute("font"), colorscheme:this.getAttribute("colorscheme"), href:this.getAttribute("href"), ref:this.getAttribute("ref"), header:this.getAttribute("header")};
			return true
		}, getUrlBits:function() {
