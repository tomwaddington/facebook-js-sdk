...
if(!FB) {
	...
	(function() {
			...
			(function() {
				...
				b(window.FB, {_apiKey:null, _authResponse:null, _userStatus:"unknown", _logging:true, _inCanvas:a, _https:function() {
					...
					if(/_fb_https?/.test(window.name)) {
						return window.name.indexOf("_fb_https") != -1
					}
		...
		__d("legacy:fb.json", ["JSON3"], function(a, b, c, d) {
			var e = b("JSON3");
			FB.provide("JSON", {stringify:e.stringify, parse:e.parse, flatten:function(f) {
				var g = {};
				for(var h in f) {
		...
		FB.provide("XD", {_origin:null, _transport:null, _callbacks:{}, _forever:{}, _xdProxyUrl:"connect/xd_proxy.php", _openerTransport:null, _openerOrigin:null, _nonOpenerOrigin:null, init:function(a) {
			...
			for(var f = 0, g = d.length;f < g;f++) {
				...
				if(b === "opener" || b === "parent" || b === "top") {
					...
					if(c = /^frames\[['"]?([a-zA-Z0-9\-_]+)['"]?\]$/.exec(b)) {
						e = e.frames[c[1]]
					}else {
						throw new SyntaxError("Malformed id to resolve: " + a + ", pt: " + b);
					}
				}
			}
			return e
			...
			var a = new RegExp("[?&]" + FB.XD.Fragment._magic + "#(.*)$"), b = location.href.match(a), c = b && b[1];
			if(c) {
		...
		__d("UserAgent", [], function(a, b, c, d, e, f) {
			var g = false, h, i, j, k, l, m, n, o, p, q;
			function r() {
				if(g) {
					return
				}
				g = true;
				var t = navigator.userAgent, u = /(?:MSIE.(\d+\.\d+))|(?:(?:Firefox|GranParadiso|Iceweasel).(\d+\.\d+))|(?:Opera(?:.+Version.|.)(\d+\.\d+))|(?:AppleWebKit.(\d+(?:\.\d+)?))/.exec(t), v = /(Mac OS X)|(Windows)|(Linux)/.exec(t), w = /\b(iPhone|iP[ao]d)/.exec(t);
				p = !!/Win64/.exec(t);
				if(u) {
		...
		__d("Log", ["sprintf"], function(a, b, c, d, e, f) {
			var g = b("sprintf"), h = Array.prototype.slice, i = {DEBUG:3, INFO:2, WARNING:1, ERROR:0};
			function j(l, m, n) {
				var o = g.apply(null, n);
				if(k.level >= l && window.console) {
					console[m in console ? m : "log"](o)
				}
			}
			var k = {level:-1, Level:i, debug:function() {
		...
		__d("legacy:fb.arbiter", ["SDKConfig"], function(a, b, c, d) {
			var e = c("SDKConfig");
			if(!e.legacy) {
				FB.provide("Arbiter", {BEHAVIOR_EVENT:"e", BEHAVIOR_PERSISTENT:"p", BEHAVIOR_STATE:"s", inform:function(f, g, h, i, j) {
					FB.XD.sendToFacebook("facebook", {method:f, params:FB.JSON.stringify(g || {}), behavior:j || FB.Arbiter.BEHAVIOR_PERSISTENT, relation:h})
				}}, true)
			}
		}, 3);
		FB.provide("Canvas", {_timer:null, _lastSize:{}, _isTabIframe:!!~window.name.indexOf("app_runner_"), _pageInfo:{clientWidth:0, clientHeight:0, scrollLeft:0, scrollTop:0, offsetLeft:0, offsetTop:0}, getPageInfo:function(a) {
		...
		FB.subclass("XFBML.IframeWidget", "XFBML.Element", null, {_iframeName:null, _showLoader:true, _refreshOnAuthChange:false, _allowReProcess:false, _fetchPreCachedLoader:false, _visibleAfter:"load", _widgetPipeEnabled:false, getUrlBits:function() {
			...
			if(FB.XD.sendToFacebook) {
				FB.XD.sendToFacebook(this.getIframeName(), {method:event, params:FB.JSON.stringify(a || {}), behavior:c || FB.Arbiter.BEHAVIOR_PERSISTENT})
			}else {
				if(this.loaded) {
					this._arbiterInform(event, a, b, c)
				}else {
					this.subscribe("iframe.onload", FB.bind(this._arbiterInform, this, event, a, c))
				}
			}
		...
		FB.subclass("XFBML.RecommendationsBar", "XFBML.IframeWidget", null, {getUrlBits:function() {
			...
			var f = FB.bind(function() {
				...
				window.setTimeout(FB.bind(this.arbiterInform, this, "platform/plugins/recommendations_bar/action", null, a, FB.Arbiter.BEHAVIOR_STATE), this._attr.read_time * 1E3);
				return true
			}, this);
		...
		FB.subclass("XFBML.Registration", "XFBML.IframeWidget", null, {_visibleAfter:"immediate", _baseHeight:167, _fieldHeight:28, _skinnyWidth:520, _skinnyBaseHeight:173, _skinnyFieldHeight:52, setupAndValidate:function() {
			...
			if(this._attr.onvalidate) {
				this.subscribe("xd.validate", this.bind(function(a) {
					var b = FB.JSON.parse(a.value), c = FB.bind(function(e) {
						this.arbiterInform("Registration.Validation", {errors:e, id:a.id}, true)
					}, this), d = FB.Helper.executeFunctionByName(this._attr.onvalidate, b, c);
					if(d) {
						c(d)
					}
				}))
		...
		FB.subclass("XFBML.SocialContext", "XFBML.IframeWidget", null, {setupAndValidate:function() {
			...
			var b = {pluginID:this.getAttribute("plugin-id"), socialContextPageIDs:JSON.parse(a.social_context_page_ids)};
		...
		void 0;
		__d("XDConfig", [], {"XdUrl":"connect/xd_arbiter.php?version=5", "Flash":{"path":"https://s-static.ak.fbcdn.net/rsrc.php/v1/ys/r/WON-TVLCpDP.swf"}, "useCdn":true});
