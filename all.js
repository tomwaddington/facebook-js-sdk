...
FB.provide("Dialog", {_dialogs:{}, _loaderEl:null, _stack:[], _active:null, get:function(a) {
...
FB.provide("UIServer", {Methods:{}, _active:{}, _defaultCb:{}, _resultToken:'"xxRESULTTOKENxx"', _forceHTTPS:false, _resizeInitialized:false, genericTransform:function(a) {
	...
	if(b.post) {
		...
		FB.UIServer.setActiveNode(b, window.open(b.url, b.id, c))
	}
	...
	if(!FB.UA.mobile() || !FB.UIServer._active[a]) {
		return
	}
	...
	for(var b in FB.UIServer._active) {
		if(FB.UIServer._active.hasOwnProperty(b) && b in FB.UIServer._defaultCb) {
			var c = FB.UIServer._active[b];
			try {
				if(c.tagName) {
					continue
