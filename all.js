window.FB || function() {
	...
	function __c() {
		...
		__d("CanvasPrefetcherConfig", [], {"blacklist":[0x83d70a64a992], "sampleRate":500})
	}
	function __d() {
	...
	(function() {
		...
		__d("Type", ["copyProperties", "Assert"], function(a, b, c, d, e, f) {
			...
			function j(n, o, p, q) {
				h.isFunction(n);
				if(o && o.prototype instanceof i === false) {
					throw new Error("parent type does not inherit from MixinTarget");
				}
				o = o || i;
				var r = new Function;
			...
			function l(n, o) {
				...
				if(n instanceof i) {
					for(var p = 0;p < n.__mixins.length;p++) {
						if(n.__mixins[p] == o) {
							return true
						}
					}
		...
		__d("sdk.ErrorHandling", ["UserAgent", "sdk.Scribe", "sdk.Runtime", "wrapFunction", "SDKConfig"], function(a, b, c, d, e, f) {
			var g = b("UserAgent"), h = b("sdk.Scribe"), i = c("SDKConfig"), j = b("sdk.Runtime"), k = b("wrapFunction"), l = false;
			function m(s) {
				typeof console !== "undefined" && typeof bagofholding === "function";
				delete s._originalError;
				h.log("jssdk_error", {appId:j.getClientID(), error:s.name || s.message, extra:s});
				throw s;
			}
			function n(s) {
				var t = {line:s.lineNumber || s.line, message:s.message, name:s.name, script:s.fileName || s.sourceURL || s.script, stack:s.stackTrace || s.stack};
				t._originalError = s;
			...
			if(q && Math.floor(Math.random() * 100) + 1 < q) {
				l = true
			}
			if(l) {
				k.setWrapper(o, "entry")
			}
			var r = {guard:o, unguard:p};
			e.exports = r
		});
		...
		__d("FB", ["guid", "dotAccess", "copyProperties", "safeEval", "wrapFunction", "sdk.ErrorHandling", "GlobalCallback", "sdk.Insights", "Log", "sdk.Scribe", "QueryString", "UrlMap", "sdk.Runtime", "SDKConfig"], function(a, b, c, d, e, f) {
					...
					default:
						return m.guard(function() {
							...
							if(la && typeof la === "object") {
								...
								for(var pa in la) {
									var qa = la[pa];
									if(typeof qa !== "function" || pa === "constructor") {
										continue
									}
									na = false;
			...
			var ba = /iframe_canvas|app_runner/.test(window.name), ca = /dialog/.test(window.name), da = function() {
				if(location.protocol == "https:" && (window == top || !(ba || ca))) {
					return true
				}
			...
			function ea(fa, ga, ha, ia) {
				for(var ja in ga) {
					if(ha || typeof fa[ja] === "undefined") {
						fa[ja] = ia ? ia(ga[ja]) : ga[ja]
					}
				}
				return fa
			}
		...
		__d("ApiClient", ["copyProperties", "flattenObject", "sprintf", "CORSRequest", "FlashRequest", "JSONPRequest", "Log", "UrlMap", "URL", "ApiClientConfig"], function(a, b, c, d, e, f) {
			...
			var z = {setAccessToken:function(aa) {
				q = aa
		...
		__d("sdk.XD", ["guid", "resolveWindow", "wrapFunction", "FB", "XDM", "Log", "QueryString", "Queue", "URL", "sdk.RPC", "sdk.Runtime", "XDConfig"], function(a, b, c, d, e, f) {
			...
			function ga(la, ma) {
				if(ma && ma !== "native" && !p(ma).isFacebookURL()) {
					return
				}
				if(typeof la == "string") {
					if(/^FB_RPC:/.test(la)) {
				...
				if(la.access_token) {
					k._https = /^https/.test(z)
				}
				if(la.cb) {
					var oa = k.XD._callbacks[la.cb];
					if(!k.XD._forever[la.cb]) {
						delete k.XD._callbacks[la.cb]
					}
					if(oa) {
						oa(la)
					}
			...
			var ka = {rpc:q, _callbacks:{}, _forever:{}, _channel:y, _origin:z, onMessage:ga, recv:ga, init:function(la, ma) {
				...
				var oa = k.Content.appendHidden(document.createElement("div")), pa = l.create({root:oa, channel:y, channelPath:"/" + g.XdUrl + "#", flashUrl:g.Flash.path, whenReady:function(qa) {
					...
					if(!k.onlyUseHttps()) {
						v = ia({url:ta + sa, name:"fb_xdm_frame_http", root:oa})
					}
		...
		FB.provide("Canvas", {_timer:null, _lastSize:{}, _isTabIframe:!!~ES5(window.name, "indexOf", true, "app_runner_"), _pageInfo:{clientWidth:0, clientHeight:0, scrollLeft:0, scrollTop:0, offsetLeft:0, offsetTop:0}, getPageInfo:function(a) {
			...
			ES5(FB.Array.toArray(b), "forEach", true, function(c) {
				var c = b[i];
				if(c.type.toLowerCase() != "application/x-shockwave-flash" && (!c.classid || c.classid.toUpperCase() != FB.Canvas._flashClassID)) {
					return
				}
				var d = false;
				for(var e = 0;e < c.childNodes.length;e++) {
					if(c.childNodes[e].nodeName.toLowerCase() == "param" && c.childNodes[e].name.toLowerCase() == "wmode") {
						if(c.childNodes[e].value.toLowerCase() == "opaque" || c.childNodes[e].value.toLowerCase() == "transparent") {
							d = true
						}
					}
				}
				if(!d) {
					...
					if(FB.Canvas._devHideFlashCallback) {
						var g = 200, h = {state:a.state, elem:c}, j = ES5(function(i) {
							if(i.state == "opened") {
								FB.Canvas.hideFlashElement(i.elem)
							}else {
								FB.Canvas.showFlashElement(i.elem)
							}
						}, "bind", true, this, h);
						setTimeout(j, g);
		...
		__d("sdk.computeContentSize", ["DOMWrapper", "createArrayFrom"], function(a, b, c, d, e, f) {
			...
			function i() {
				...
				if(k.offsetWidth < k.scrollWidth) {
					...
					ES5(h(k.childNodes), "forEach", true, function(u) {
		...
		__d("sdk.Canvas.IframeHandling", ["sdk.Runtime", "Log", "sdk.RPC", "sdk.computeContentSize"], function(a, b, c, d, e, f) {
			...
			function m(p) {
				if(!g.getInitialized() && arguments.callee.caller != n) {
					h.warn("FB.init is required for setSize to take effect")
				}
				if(typeof p != "object") {
					p = {}
				}
			...
			function n(p, q) {
				if(!g.getInitialized()) {
					h.warn("FB.init is required for setAutoGrow to take effect")
				}
		...
		__d("sdk.Canvas.Environment", ["sdk.Runtime", "Log", "sdk.RPC"], function(a, b, c, d, e, f) {
			...
			function j(m) {
				...
				if(!g.getInitialized()) {
					h.warn("FB.init is required for getPageInfo to take effect")
				}
			...
			function k(m, n) {
				if(!g.getInitialized()) {
					h.warn("FB.init is required for scrollTo to take effect")
				}
		...
		__d("sdk.Canvas.Navigation", ["sdk.RPC", "Log", "sdk.Runtime"], function(a, b, c, d, e, f) {
			...
			function l(n) {
				if(!i.getInitialized()) {
					h.warn("FB.init is required for setUrlHandler to take effect")
				}
		...
		__d("legacy:fb.canvas", ["FB", "sdk.Canvas", "SDKConfig"], function(a, b, c, d) {
			var e = b("FB"), f = c("SDKConfig"), g = b("sdk.Canvas");
			if(f.migrate) {
				e.provide("Canvas", g);
				e.provide("CanvasInsights", {setDoneLoading:g.setDoneLoading})
			}
		}, 3);
		...
		FB.provide("Type", {isType:function(a, b) {
			while(a) {
				if(a.constructor === b || a === b) {
					...
					a = a._base || a.constructor.prototype._base
				}
			}
			return false
		}});
		...
		__d("legacy:fb.canvas.prefetcher", ["FB", "sdk.Canvas.Prefetcher", "SDKConfig"], function(a, b, c, d) {
			var e = b("FB"), f = c("SDKConfig"), g = b("sdk.Canvas.Prefetcher");
			if(f.migrate) {
				e.provide("Canvas.Prefetcher", g)
			}
		}, 3);
		FB.Dom.ready(function() {
		...
		FB.provide("", {initSitevars:{}, init:function(a) {
			...
			if(!FB._initialized) {
				...
				FB.Runtime.subscribe("AccessToken.change", function(d) {
					if(!d && FB.Runtime.getLoginStatus() === "connected") {
						FB.getLoginStatus(null, true)
					}
				})
			}
		...
		FB.subclass("Waitable", "Obj", function() {
			...
			if(b) {
				this.subscribe("error", FB.unguard(b))
			}
